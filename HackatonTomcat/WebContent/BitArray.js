...
// resize the bit array (append new false/0 to end) 
...
// resize the bit array (append new false/0 to begin) 
BitArray.prototype.pushOff = function (newSize) {
    var tmp = new Array();
    for (var i = 0; i < newSize; i++) {		
        if (i >= newSize - this.m_bits.length) {
            tmp.push(this.m_bits[i-(newSize - this.m_bits.length)]);
        } else {
            tmp.push(BitArray._OFF);
        }
    }
...
// Convert a String into a bit array
BitArray.fromString = function (string) {
    var bits = new BitArray();
	for(var i in string){
		bits.m_bits.push(parseInt(string[i]) ? BitArray._ON : BitArray._OFF);
	}
	return bits;
};
